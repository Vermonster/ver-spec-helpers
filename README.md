...
specs:
  - id: census-management
    domain: census
    summary: "CRUD rules and unique constraints for the censuses table (one per provider/date/location)."
    token_estimate: 420
    paths:
      - services/census/
      - lib/db/schema/census.ts
  - id: clinical-core-patterns
    domain: clinical
    summary: "clinicalCols() vs tenantCols() helpers; meta_source semantics for EHR-imported records."
    token_estimate: 310
    paths:
      - lib/db/schema/helpers.ts
```
